<template>
  <div class="mt-4">
    ...
    <div class="overflow-auto max-h-128">
      ...
      <!--              START EXPENDITURES SECTION-->
      <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
          <div class="md:col-span-1">
            <div class="px-4 sm:px-0">
              <h3 class="text-lg font-medium leading-6 text-gray-900">Expenditures</h3>
              <p class="mt-1 text-sm text-gray-600">Taxes, commissions, gifts etc .</p>
            </div>
          </div>
          <div class="mt-3 md:mt-0 md:col-span-2">
            <slot name="expenditures">NO ANY INCOMES!</slot>
            <MoneyTransferForm
              v-if="isExpensesFormOpen"
              :month="month"
              @add="handleAddExpenditure"
              @cancel="cancelExpenditureTransaction"
            >
              Adding Expenditure
            </MoneyTransferForm>
        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
          ...
          <div class="mt-5 md:mt-0 md:col-span-2">
            <TransactionsSummaries
              ...
              :expenditures="[...tempExpenditures, ...expenditures]"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleAddExpenditure(obj) {
      this.isExpensesFormOpen = false;
      this.$emit('addExpenditure', obj);
    },
    cancelExpenditureTransaction() {
      this.isExpensesFormOpen = false;
    },
